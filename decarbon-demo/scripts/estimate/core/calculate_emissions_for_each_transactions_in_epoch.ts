 ...
 */
export default async function calculate_emissions_of_transactions_in_epoch(_epoch: Epoch, _transactionList: Transaction[], _addressToAccount: Map<string, Account>)
    : Promise<boolean> {
    ...
    try {
        ...
        for (let i = 0; i <= _epoch.blocks!.length - 1; i++) {
            ...
            if (transactionReceipts === undefined || transactionResponses === undefined) {
                return false;
            }

            if (transactionResponses.length != transactionReceipts.length) {
                throw new Error("transactionResponses.length != transactionReceipts.length");
            }
            ...
            for (let i = 0; i < transactionReceipts.length; i++) {
                ...
                const transaction__sender: string = transactionResponses[i].from as string;
                const transaction__receiver: string = transactionResponses[i].to as string;
                if (_addressToAccount.has(transaction__sender) || _addressToAccount.has(transaction__receiver)) {
                    const transaction__gasUsed: bigint = BigInt(hex2Dec(transactionReceipts[i].gasUsed as string));
                    const transaction__gasPrice: bigint = BigInt(hex2Dec(transactionResponses[i].gasPrice as string));
                    const transaction__value: bigint = BigInt(hex2Dec(transactionResponses[i].value as string));
                    ...
                    const transaction__blockNumber: number = hex2Dec(transactionResponses[i].blockNumber as string);
                    ...
                    const newTransaction: Transaction = {
                        hash: transactionResponses[i].hash as string,
                        nonce: hex2Dec(transactionResponses[i].nonce as string),
                        transaction_index: hex2Dec(transactionResponses[i].transactionIndex as string),
                        from_address: transaction__sender,
                        to_address: transaction__receiver,
                        value: transaction__value,
                        gas: BigInt(hex2Dec(transactionResponses[i].gas as string)),
                        gas_price: transaction__gasPrice,
                        block_number: hex2Dec(transactionResponses[i].blockNumber as string),
                        block_hash: transactionResponses[i].blockHash as string,
                        block_timestamp: transaction__timestamp,
                        receipt_contract_address: (transactionReceipts[i].contractAddress as string) ?? "",
                        receipt_status: hex2Dec(transactionReceipts[i].status as string),
                        receipt_gas_used: transaction__gasUsed,
                        receipt_cumulative_gas_used: BigInt(hex2Dec(transactionReceipts[i].cumulativeGasUsed as string)),
...
// calculate_emissions_of_transactions_in_epoch(exampleEpoch, transactionList, addressList)
//     .then(() => {
