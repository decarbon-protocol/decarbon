...
export default async function get_tx_from_tx_hashes(transactionHashes: string[]): Promise<[any[], any[]]> {
    const batchRequests = transactionHashes.flatMap((hash, index) => [
        {
            jsonrpc: '2.0',
    ...
    try {
        ...
        const receiptResults = response.data.filter((result: any, index: number) => index % 2 === 0).map((result: any) => result.result);
        const transactionResults = response.data.filter((result: any, index: number) => index % 2 === 1).map((result: any) => result.result);
...
// ]
// get_tx_from_tx_hashes(mocks)
//     .then(([receipts, transactions]) => {
